"""
Money muling detection scoring methodology
...
class RiskWeights:
    # === PATTERN BASE SCORES (confidence-weighted) ===
    CYCLE_INVOLVEMENT = 85.0        # Circular routing - strongest indicator (95% confidence)
    SMURFING_FAST_REDI = 75.0       # Fan-in + <24h redistribution (85% confidence)
    SMURFING_DELAYED_REDI = 55.0    # Fan-in + 24-96h redistribution (70% confidence)
    SMURFING_SLOW_REDI = 40.0       # Fan-in but >96h (possibly merchant) (50% confidence)
    SHELL_ACCOUNT = 60.0            # Pass-through with 2-3 transactions (75% confidence)
    
    # === CONFIDENCE THRESHOLDS ===
    HIGH_CONFIDENCE = 0.85          # Pattern must be >85% confident to apply full weight
    MEDIUM_CONFIDENCE = 0.65        # 65-85% confidence = partial weight
    LOW_CONFIDENCE = 0.50           # <50% confidence = discard signal
    
    # === TEMPORAL MULTIPLIERS (velocity-based) ===
...
def calculate_final_score(
    ...
    """
    Scoring methodology with enhanced false positive control
    ...
    # STEP 4: FALSE POSITIVE CONTROL - Compliance rules
    # ===== MERCHANT PROTECTION =====
